    ...
    async def _process(self, url: str):
        ...
        try:
            ...
            if size_bytes <= limit_bytes:
                ...
                await dm.send(f"⚠️ الملف كبير جدًا ({size_bytes/1024/1024:.1f}MB). الحد المسموح {MAX_UPLOAD_MB}MB.")
...
class PanelView(discord.ui.View):
    ...
    @discord.ui.button(label="اضـغـط لـتـحـمـيـل مـقـطـع", style=discord.ButtonStyle.primary)
    async def start(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(URLModal(requester=interaction.user))

# ========= أوامر البوت =========
@bot.tree.command(name="setup_panel", description="𝐍𝐗𝐒 𝐕𝐈𝐃𝐄𝐎 𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃𝐄𝐑 ╾━╤デ╦︻")
@app_commands.checks.has_permissions(manage_guild=True)
async def setup_panel(interaction: discord.Interaction):
    await interaction.response.send_message(
        "✅ تم إنشاء لوحة التحميل. يستطيع الجميع استخدامها لتحميل المقاطع:",
        view=PanelView()
    )
